...
public class Validator {
    ...
    public static Map<String, String> WithdrawDTO(AccountWithdraw withdraw){
        Map<String, String> errors = new HashMap<>();


        if (withdraw.iban() == null || !withdraw.iban().trim().matches("GR\\d{3,25}")){
            errors.put("iban", "Το iban πρέπει να ξεκινάει με GR και να ακολουθείται από 3-25 ψηφία.");
        }

        if (withdraw.amount() == null || withdraw.amount().compareTo(BigDecimal.ZERO) < 0) {
