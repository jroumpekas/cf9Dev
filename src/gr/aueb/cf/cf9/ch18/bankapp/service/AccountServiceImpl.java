        ...
        try {
            Account account = accountDAO.findByIban(depositDTO.iban())
                    .orElseThrow(() -> new AccountNotFoundException(
                            "Account with iban " + depositDTO.iban() + " not found"
                    ));

            // ✅ ΚΑΝΟΥΜΕ ΤΗΝ ΚΑΤΑΘΕΣΗ
            account.setBalance(account.getBalance().add(depositDTO.amount()));
            accountDAO.saveOrUpdate(account);

        } catch (AccountNotFoundException e) {
            System.err.printf("%s. The account with iban=%s was not found!%n",
                    LocalDateTime.now(), depositDTO.iban());
            throw e; // ✅ αφού η μέθοδος δηλώνει throws
        }
    }

    @Override
    public void withdraw(AccountWithdraw withdrawDTO)
            throws InsufficientBalanceException, AccountNotFoundException {

        try {
            Account account = accountDAO.findByIban(withdrawDTO.iban())
                    .orElseThrow(() -> new AccountNotFoundException(
                            "Account with iban " + withdrawDTO.iban() + " not found!"
                    ));

            if (account.getBalance().compareTo(withdrawDTO.amount()) < 0) {
                throw new InsufficientBalanceException(
                        "Invalid amount " + withdrawDTO.amount() + " for account with iban "
                                + withdrawDTO.iban() + " (greater than balance)"
                );
            }
            ...
            System.err.printf("%s. The amount=%s is greater than the balance for iban=%s.%n",
